...
public class UserServlet extends BaseServlet
{
    ...
    public void register(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException
    {
        ...
        if (result)
        {
            ...
            String emailMsg = user.getUsername() + "用户恭喜您注册成功，请点击下面的连接进行激活账户" + "<a href='http://localhost:8080/shop/user?method=active&activeCode=" + code + "'>"
                    + "http://localhost:8080/shop/user?method=active&activeCode=" + code + "</a>";
            String topic = "用户激活";
            try
            {
                MailUtil.sendMail(email, emailMsg, topic);
            }
            catch (Exception e)
            {
