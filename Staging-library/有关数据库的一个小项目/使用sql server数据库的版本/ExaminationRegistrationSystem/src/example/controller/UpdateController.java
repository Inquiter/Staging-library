                    ...
                    switch (table) {
                        case "用户信息":
                            if (datas.length < 2 || datas.length > 5) {
                                ...
                                preparedStatement = connection.prepareStatement("select id from users");
                                ...
                                if (!ids.contains(datas[0])) resp.getWriter().write("id不存在!");
                                    ...
                                    preparedStatement = connection.prepareStatement("update users set username = ?,password = ?,birthday = ?,idcard = ?,account_type = ? where id = ?");
